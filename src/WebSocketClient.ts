...
export default class WebSocketClient extends EventEmitter {
  ...
  constructor(opts: WebSocketOpts) {
    ...
    opts.reconnect = Object.assign(
      {},
      defaultReconnectOpts,
      opts.reconnect || {}
    );
  ...
  private authenticate(): void {
    ...
    const authMsg = ProtobufClient.ClientMessage.create({
      apiAuthentication: ProtobufClient.APIAuthenticationMessage.create({
        ...
        token: getToken(
          this.opts.creds.apiKey,
          this.opts.creds.secretKey,
          nonce
        ),
        version
      })
  ...
  private authResultHandler(
    authResult: ProtobufStream.IAuthenticationResult
  ): void {
...
const errConnNotReady = new Error(
